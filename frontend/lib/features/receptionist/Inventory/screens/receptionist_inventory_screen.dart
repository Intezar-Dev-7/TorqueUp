...
class ReceptionistInventoryScreen extends StatefulWidget {
  ...
  @override
  State<ReceptionistInventoryScreen> createState() => _ReceptionistInventoryScreenState();
}

class _ReceptionistInventoryScreenState extends State<ReceptionistInventoryScreen> {
  String searchQuery = "";
  ...
  void _editInventoryProductsForm(BuildContext context, {required String productId, required int productQuantity, required int productPrice, required String productStatus}) {
    showDialog(
      context: context,
      builder: (context) => Dialog(
        backgroundColor: Colors.transparent,
  ...
  List<Inventory> _getFilteredInventory(List<Inventory> allInventory) {
    ...
    return allInventory.where((item) => item.productName.toLowerCase().contains(searchQuery.toLowerCase())).toList();
  }
              ...
              children: [
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(Icons.inventory_2_outlined, color: AppColors.sky_blue, size: 24),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Inventory', style: TextStyle(fontSize: isMobile(screenWidth) ? 18 : 22, color: AppColors.text_dark, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 2),
                            Text('Manage your inventory', style: TextStyle(fontSize: isMobile(screenWidth) ? 12 : 14, color: AppColors.text_grey, fontWeight: FontWeight.w400)),
                          ],
                        ),
                      ),
                    ],
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.transparent, shadowColor: AppColors.transparent, padding: EdgeInsets.symmetric(vertical: 12, horizontal: isMobile(screenWidth) ? 12 : 20), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    onPressed: () => _openInventoryBookingForm(context),
                    child: isMobile(screenWidth)
                        ? Icon(Icons.add_rounded, color: AppColors.white)
                        : Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.add_rounded, color: AppColors.white), const SizedBox(width: 8), Text('Add Product', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.w600))]),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildStatsCards(double screenWidth, List<Inventory> inventoryList) {
    if (isMobile(screenWidth)) {
      return Column(
        children: [
          Row(
            children: [
              Expanded(child: _buildInventoryCard(title: 'Total Products', value: inventoryList.length, icon: Icons.inventory_2_outlined, color: AppColors.sky_blue)),
              const SizedBox(width: 12),
              Expanded(child: _buildInventoryCard(title: 'Low Stock', value: inventoryList.where((item) => item.productStatus == 'Low Stock').length, icon: Icons.warning_amber_rounded, color: AppColors.status_pending)),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(child: _buildInventoryCard(title: 'Out of Stock', value: inventoryList.where((item) => item.productStatus == 'Out of Stock').length, icon: Icons.hourglass_bottom_outlined, color: AppColors.error)),
              const SizedBox(width: 12),
              Expanded(child: _buildInventoryCard(title: 'Total Value', value: inventoryList.fold(0, (sum, item) => sum + item.productPrice), icon: Icons.attach_money_outlined, color: AppColors.status_completed)),
            ],
          ),
        ],
      );
    ...
    return Wrap(
      ...
      runSpacing: 16,
      children: [
        _buildInventoryCard(title: 'Total Products', value: inventoryList.length, icon: Icons.inventory_2_outlined, color: AppColors.sky_blue),
        _buildInventoryCard(title: 'Low Stock', value: inventoryList.where((item) => item.productStatus == 'Low Stock').length, icon: Icons.warning_amber_rounded, color: AppColors.status_pending),
        _buildInventoryCard(title: 'Out of Stock', value: inventoryList.where((item) => item.productStatus == 'Out of Stock').length, icon: Icons.hourglass_bottom_outlined, color: AppColors.error),
        _buildInventoryCard(title: 'Total Value', value: inventoryList.fold(0, (sum, item) => sum + item.productPrice), icon: Icons.attach_money_outlined, color: AppColors.status_completed),
      ],
    );
  }

  Widget _buildInventoryCard({required String title, required int value, required IconData icon, required Color color}) {
    return Container(
        ...
        child: Column(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.sky_blue.withOpacity(0.08), borderRadius: const BorderRadius.only(topLeft: Radius.circular(16), topRight: Radius.circular(16))),
              child: Row(
                children: [
                  Expanded(flex: 3, child: Text('Product Name', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.sky_blue, fontSize: 14))),
                  Expanded(flex: 1, child: Text('Quantity', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.sky_blue, fontSize: 14))),
                  Expanded(flex: 1, child: Text('Price', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.sky_blue, fontSize: 14))),
                  Expanded(flex: 1, child: Text('Status', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.sky_blue, fontSize: 14))),
                  Expanded(flex: 1, child: Text('Actions', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.sky_blue, fontSize: 14))),
                ],
              ),
            ),
            if (provider.isLoading)
              const Expanded(child: Center(child: CircularProgressIndicator(color: Color(0xFF4FC3F7))))
    ...
    return Padding(
      ...
      child: Row(
        children: [
            ...
            child: Row(
              children: [
                Container(
                  width: 48,
                  height: 48,
                  decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: AppColors.sky_blue.withOpacity(0.3), width: 2)),
                  child: CircleAvatar(
          ...
          Expanded(flex: 1, child: Text('\$${item.productPrice.toDouble().toStringAsFixed(2)}', style: TextStyle(color: AppColors.text_dark, fontSize: 14, fontWeight: FontWeight.w500))),
              ...
              children: [
                Container(
                  ...
                  child: IconButton(
                    ...
                    icon: Icon(Icons.edit_outlined, size: 18, color: AppColors.status_completed),
                    onPressed: () => _editInventoryProductsForm(context, productId: item.productId, productQuantity: item.productQuantity, productPrice: item.productPrice, productStatus: item.productStatus),
                  ),
                ),
                ...
                Container(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () => _showDeleteDialog(item.productId, provider),
                  ),
                ),
              ],
            ),
  ...
  void _showDeleteDialog(String productId, InventoryProvider provider) {
    showDialog(
      context: context,
