...
import 'package:frontend/features/receptionist/Staff/widgets/mechanics_widget.dart';
...
class ReceptionistStaffScreen extends StatefulWidget {
  ...
  @override
  State<ReceptionistStaffScreen> createState() => _ReceptionistStaffScreenState();
}
...
class _ReceptionistStaffScreenState extends State<ReceptionistStaffScreen> {
  ...
  void initState() {
    super.initState();
    // Load all staff only once from the top screen
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: Icon(Icons.groups_rounded, color: AppColors.sky_blue, size: 24),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Staff', style: TextStyle(fontSize: isMobile(screenWidth) ? 18 : 22, color: AppColors.text_dark, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 2),
                            Text('Edit, Update and Delete employees', style: TextStyle(fontSize: isMobile(screenWidth) ? 12 : 14, color: AppColors.text_grey, fontWeight: FontWeight.w400)),
                          ],
                        ),
                      ),
                    ],
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    ...
                    child: isMobile(screenWidth)
                        ...
                        : Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.add_rounded, color: AppColors.white), const SizedBox(width: 8), Text('Add Staff', style: TextStyle(color: AppColors.white, fontWeight: FontWeight.w600))]),
                  ),
                ),
              ],
            ),
  ...
  Widget _buildResponsiveLayout(double screenWidth) {
    if (isMobile(screenWidth)) {
      return SingleChildScrollView(
        child: Column(
          children: const [Mechanics(), SizedBox(height: 20), Interns(), SizedBox(height: 20), OtherEmployees()],
        ),
      );
      ...
      return SingleChildScrollView(
        child: Column(
          children: [
            Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: const [Expanded(child: Mechanics()), SizedBox(width: 20), Expanded(child: Interns())],
            ),
            const SizedBox(height: 20),
            Container(width: double.infinity, constraints: BoxConstraints(maxWidth: screenWidth / 2 - 10), child: const OtherEmployees()),
          ],
        ),
      );
    } else {
      return Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: const [Expanded(child: Mechanics()), SizedBox(width: 20), Expanded(child: Interns()), SizedBox(width: 20), Expanded(child: OtherEmployees())],
      );
    }
  }
}